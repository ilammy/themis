...
let onRuntimeInitialized: (value: void | PromiseLike<void>) => void

// The current facility is "async initialize()" function which user code
// is expected to call, then await the result to resolve before using other
// WasmThemis functions.
//
// Calling initialize() kicks off the download and compilation process.
// This also allows to pass some extra parameters to Emscripten stub,
// such as the URL from where "libthemis.wasm" is to be obtained.
...
// TypeScript does not allow to extend Promise nicely, but since JavaScript
