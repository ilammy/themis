...
{
    if (private_result != THEMIS_SUCCESS && private_result != THEMIS_BUFFER_TOO_SMALL) {
        soter_wipe(private_key, *private_key_length);
    }
    if (public_result != THEMIS_SUCCESS && public_result != THEMIS_BUFFER_TOO_SMALL) {
...
themis_status_t themis_gen_sym_key(uint8_t* key, size_t* key_length)
{
    ...
    /*
     * soter_rand() wipes the output on failure, soter_wipe() not needed.
     */
